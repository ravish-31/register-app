pipeline {
    ...
    stages {
        ...
        stage("Checkout from SCM") {
            steps {
                git branch: 'main', credentialsId: 'github', url: 'https://github.com/ravish-31/register-app'
            }
        }
        ...
        stage("Trigger CD Pipeline") {
            steps {
                script {
                    ...
                    sh 'curl -v -k --user admin:$JENKINS_API_TOKEN -X POST "http://13.233.163.187:8080/job/gitops-register-app-ci/buildWithParameters?token=pipeline-trigger"'
                    }
